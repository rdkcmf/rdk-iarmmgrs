...
{
    __TIMESTAMP();
    LOG("Reset: IARM-Release MEDIA Focus for RI Process\n");
...
static void grabFocus()
{
    ...
    __TIMESTAMP();
    LOG("Reset: IARM-Request MEDIA Focus for Reset Process\n");
...
void blinkLEDs(void)
{
    ...
    {
        __TIMESTAMP();
        LOG("An exception caught during FP LED Blink Operation: %s",e.getMessage().c_str());
    }
    return;
}
...
int checkResetModeSequence(int keyType, int keyCode, resetType_t resetMode)
{
    ...
    if( (pCurrentState->keyMode == keyType) && (pCurrentState->Key == keyCode)/* && (true == gKeyAllowed[resetMode]) */)
    {
        ...
        if(0 != pCurrentState->validTime)
        {
            ...
            __TIMESTAMP();
            LOG("Reset: gTimeOutValue: %d- %d ",gTimeOutValue,gCurrentTime);
        }
